...
isAppRunning()
{
    ...
    # Check if user want to detect html5 fullscreen on Vivaldi, modify variable vivaldi_html5_detection if you dont want Vivaldi html5 detection.
    if [ $vivaldi_html5_detection == 1 ];then
        if [[ "$activ_win_title" = *vivaldi* ]];then
            # Check if Vivaldi html5 process is running
